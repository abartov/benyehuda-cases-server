...
class User < ActiveRecord::Base
  ...
  scope :all_volunteers, lambda {
    where('(users.is_volunteer = 1 OR is_editor = 1 OR is_admin = 1) AND activated_at IS NOT NULL AND disabled_at IS NULL')
  }

  scope :editors, -> { where('is_editor = 1 AND activated_at IS NOT NULL AND disabled_at IS NULL') }
  scope :all_editors, lambda {
    where('(is_editor = 1 OR is_admin = 1) AND activated_at IS NOT NULL AND disabled_at IS NULL')
  }

  scope :admins, -> { where(is_admin: true) }

  scope :enabled, -> { where('users.disabled_at IS NULL') }
  scope :not_on_break, lambda {
    where('users.is_volunteer = 1 AND users.on_break = 0 AND users.disabled_at IS NULL AND users.activated_at IS NOT NULL')
  }
  ...
  def has_no_credentials?
    ...
    crypted_password.blank? && !activated_at.blank?
  end
  ...
  alias deliver_activation_instructions_without_db_update! deliver_activation_instructions!
  alias deliver_activation_instructions! deliver_activation_instructions_with_db_update!
  ...
  def vol_active_in_last_n_months?(n)
    assignment_histories.with_task.where.order('updated_at desc').each do |h|
      return true if h.updated_at > n.months.ago
    end
    false
  end
  ...
  def self.vols_newer_than(t)
    User.all_volunteers.where(activated_at: t..Date.today)
  end

  def self.vols_created_between(fromdate, todate)
    User.all_volunteers.where(activated_at: fromdate..todate)
  end
