namespace :tasks do
  ...
  def find_idle_tasks(idle_since)
    ...
    idle_tasks = if active_task_ids.empty?
                   candidate_tasks.includes(:assignee, :editor, :documents)
                 else
                   candidate_tasks.where.not(id: active_task_ids).includes(:assignee, :editor, :documents)
                 end
